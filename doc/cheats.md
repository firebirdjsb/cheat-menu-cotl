## Available Cheats  
---  

### **All Rituals**  
**Category**: Cult  
**Type**: Mode  
**Description**: While enabled you will have access to all rituals (including both sides of every pair)  


---  


### **Allow Shrine Creation**  
**Category**: Cult  
**Type**: Mode  
**Description**: Allows the Shrine to be created from the building menu  


---  


### **Auto Clear Ritual Cooldowns**  
**Category**: Cult  
**Type**: Mode  
**Description**: Set ritual cooldowns to zero while active  


---  


### **Build All Structures**  
**Category**: Cult  
**Type**: Simple  
**Description**: Instantly build all structures  


---  


### **Change Doctrines**  
**Category**: Cult  
**Type**: Mode  
**Description**: Allows the modification of selected doctrines  


---  


### **Change Rituals**  
**Category**: Cult  
**Type**: Mode  
**Description**: Lets you change the selected Rituals along with unlocking not yet acquired ones  


---  


### **Clear All Doctrines**  
**Category**: Cult  
**Type**: Simple  
**Description**: Clears all docterine categories and rewards (Apart from special rituals)  


---  


### **Clear Base Rubble**  
**Category**: Cult  
**Type**: Simple  
**Description**: Removes any stones and large rubble  


---  


### **Clear Base Trees**  
**Category**: Cult  
**Type**: Simple  
**Description**: Removes all trees in the base  


---  


### **Clear Dead bodies**  
**Category**: Cult  
**Type**: Simple  
**Description**: Clears any dead bodies on the floor, giving follower meat!  


---  


### **Clear Outhouses**  
**Category**: Cult  
**Type**: Simple  
**Description**: Clears all outhouses of poop and adds the contents to your inventory.  


---  


### **Clear Poop**  
**Category**: Cult  
**Type**: Simple  
**Description**: Clear any poop on the floor, giving the fertilizer directly!  


---  


### **Clear Vomit**  
**Category**: Cult  
**Type**: Simple  
**Description**: Clear any vomit on the floor!  


---  


### **Free Building Mode**  
**Category**: Cult  
**Type**: Mode  
**Description**: Buildings can be placed for free  


---  


### **Rename Cult**  
**Category**: Cult  
**Type**: Simple  
**Description**: Bring up the UI to rename the cult  


---  


### **Teleport to Cult**  
**Category**: Cult  
**Type**: Simple  
**Description**: Teleports the player to the Base  


---  


### **Unlock All Structures**  
**Category**: Cult  
**Type**: Simple  
**Description**: Unlocks all buildings  


---  


### **Clear Faith**  
**Category**: Follower  
**Type**: Simple  
**Description**: Set the current faith to zero  


---  


### **Convert Dissenting Followers**  
**Category**: Follower  
**Type**: Simple  
**Description**: Converts dissenting followers back to regular followers  


---  


### **Kill All Followers**  
**Category**: Follower  
**Type**: Simple  
**Description**: Kills all followers at the Base  


---  


### **Max Faith**  
**Category**: Follower  
**Type**: Simple  
**Description**: Clear the cult's thoughts and gives them large positive ones  


---  


### **Remove Hunger**  
**Category**: Follower  
**Type**: Simple  
**Description**: Clears starvation from any followers and maximazes satiation for all followers  


---  


### **Remove Sickness**  
**Category**: Follower  
**Type**: Simple  
**Description**: Clears sickness from all followers, cleanups any vomit, poop or dead bodies and clears outhouses  


---  


### **Revive All Followers**  
**Category**: Follower  
**Type**: Simple  
**Description**: Revive all currently dead followers  


---  


### **Spawn 'Arrived' Follower**  
**Category**: Follower  
**Type**: Simple  
**Description**: Spawns a follower ready for indoctrination  


---  


### **Spawn Follower (Worker)**  
**Category**: Follower  
**Type**: Simple  
**Description**: Spawns and auto-indoctrinates a follower as a worker  


---  


### **Spawn Follower (Worshipper)**  
**Category**: Follower  
**Type**: Simple  
**Description**: Spawns and auto-indoctrinates a follower as a worshipper  


---  


### **Add x1 Black Heart**  
**Category**: Health  
**Type**: Simple  
**Description**: Adds a Black Heart to the Player  


---  


### **Add x1 Blue Heart**  
**Category**: Health  
**Type**: Simple  
**Description**: Adds a Blue Heart to the Player  


---  


### **Die**  
**Category**: Health  
**Type**: Simple  
**Description**: Kills the Player  


---  


### **Godmode**  
**Category**: Health  
**Type**: Mode  
**Description**: Gives Invincibility  


---  


### **Heal x1**  
**Category**: Health  
**Type**: Simple  
**Description**: Heals a Red Heart of the Player  


---  


### **Follower Debug**  
**Category**: Misc  
**Type**: Mode  
**Description**: Shows Follower Debug Information  


---  


### **FPS Debug**  
**Category**: Misc  
**Type**: Mode  
**Description**: Displays the built-in FPS Debug menu  


---  


### **Hide/Show UI**  
**Category**: Misc  
**Type**: Mode  
**Description**: Show/Hide the UI of the game  


---  


### **Noclip**  
**Category**: Misc  
**Type**: Mode  
**Description**: Collide with nothing!  


---  


### **Skip Hour**  
**Category**: Misc  
**Type**: Simple  
**Description**: Skip an hour of game time  


---  


### **Structure Debug**  
**Category**: Misc  
**Type**: Mode  
**Description**: Shows Structure Debug Information  


---  


### **Give Big Gift**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives you a 'big' gift x10  


---  


### **Give Commandment Stone**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives a Commandment Stone  


---  


### **Give Fertiziler**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives x100 Fertiziler (Poop)  


---  


### **Give Fish**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives all types of fish (x10)  


---  


### **Give Follower Meat**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives x10 Follower Meat  


---  


### **Give Follower Necklaces**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives one of each of the various follower necklaces  


---  


### **Give Food**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives all farming based foods  


---  


### **Give Monster Heart**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives a heart of the heretic  


---  


### **Give Resources**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives 100 of the main primary resources  


---  


### **Give Small Gift**  
**Category**: Resources  
**Type**: Simple  
**Description**: Gives you a 'small' gift x10  


---  


### **Weather: Clear**  
**Category**: Weather  
**Type**: Simple  
**Description**: Set weather to clear  


---  


### **Weather: Rain**  
**Category**: Weather  
**Type**: Simple  
**Description**: Set weather to raining  


---  


### **Weather: Snow**  
**Category**: Weather  
**Type**: Simple  
**Description**: Set weather to snowing (winter)  


---  


### **Weather: Windy**  
**Category**: Weather  
**Type**: Simple  
**Description**: Set weather to windy  


---  


### **Auto Shear Wool**  
**Category**: DLC  
**Type**: Mode  
**Description**: Automatically shears wool from all Lambish and Yakish followers  


---  


### **Clear Base Grass**  
**Category**: Cult  
**Type**: Simple  
**Description**: Removes all grass and weeds in the base  


---  



[Back to Home](../README.md)
