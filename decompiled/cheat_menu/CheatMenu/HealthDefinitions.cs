...
namespace CheatMenu
{
	...
	{
		[CheatDetails("Godmode", "Godmode (OFF)", "Godmode (ON)", "Full invincibility - no damage taken", true, 0)]
		public static void GodMode(bool flag)
		{
			foreach (PlayerFarming playerFarming in PlayerFarming.players)
			{
				playerFarming.health.GodMode = (flag ? Health.CheatMode.God : Health.CheatMode.None);
			}
			CultUtils.PlayNotification(flag ? "Godmode ON!" : "Godmode OFF!");
		}

		[CheatDetails("Demigod Mode", "Demigod (OFF)", "Demigod (ON)", "Take damage but cannot die below 1 HP", true, 0)]
		public static void DemiGodMode(bool flag)
		{
			foreach (PlayerFarming playerFarming in PlayerFarming.players)
			{
				playerFarming.health.GodMode = (flag ? Health.CheatMode.Demigod : Health.CheatMode.None);
			}
			CultUtils.PlayNotification(flag ? "Demigod ON!" : "Demigod OFF!");
		}

		[CheatDetails("Immortal Mode", "Immortal (OFF)", "Immortal (ON)", "Cannot die at all even at 0 HP", true, 0)]
		public static void ImmortalMode(bool flag)
		{
			foreach (PlayerFarming playerFarming in PlayerFarming.players)
			{
				playerFarming.health.GodMode = (flag ? Health.CheatMode.Immortal : Health.CheatMode.None);
			}
			CultUtils.PlayNotification(flag ? "Immortal ON!" : "Immortal OFF!");
		}

		[CheatDetails("Heal x1", "Heals a Red Heart of the Player", false, 0)]
		...
		[CheatDetails("Full Heal", "Fully heals the Player to max HP", false, 0)]
		...
		[CheatDetails("Add x1 Red Heart", "Permanently adds a Red Heart container", false, 0)]
		...
		[CheatDetails("Add x1 Blue Heart", "Adds a Blue Heart to the Player", false, 0)]
		...
		[CheatDetails("Add x1 Black Heart", "Adds a Black Heart to the Player", false, 0)]
		...
		[CheatDetails("Add x1 Spirit Heart", "Adds a full Spirit Heart to the Player", false, 0)]
		...
		[CheatDetails("Add x1 Fire Heart", "Adds a Fire Heart to the Player", false, 0)]
		public static void AddFireHeart()
		{
			if (PlayerFarming.Instance != null)
			{
				PlayerFarming.Instance.health.FireHearts += 2f;
				CultUtils.PlayNotification("Fire heart added!");
			}
		}

		[CheatDetails("Add x1 Ice Heart", "Adds an Ice Heart to the Player", false, 0)]
		public static void AddIceHeart()
		{
			if (PlayerFarming.Instance != null)
			{
				PlayerFarming.Instance.health.IceHearts += 2f;
				CultUtils.PlayNotification("Ice heart added!");
			}
		}

		[CheatDetails("Unlimited HP", "Unlimited HP (OFF)", "Unlimited HP (ON)", "Uses the game's built-in accessibility unlimited HP option", true, 0)]
		public static void UnlimitedHP(bool flag)
		{
			...
			{
				Debug.LogWarning("Failed to toggle unlimited HP: " + ex.Message);
				CultUtils.PlayNotification("Failed to toggle unlimited HP!");
			}
		}

		[CheatDetails("Die", "Kills the Player instantly", false, 0)]
